...
public class ProductServiceImpl implements ProductService {
	...
	@Override
	public ProductDto updateProducts(String productJson, MultipartFile file) {
		LOG.info("Received request to update product: {}", productJson);
		if (productJson == null || productJson.trim().isEmpty()) {
			LOG.error("Invalid product JSON: JSON is null or empty");
			throw new ProductException("Product JSON cannot be null or empty");
		}
		...
		Product savedProduct = pr.save(p);
		LOG.info("Product updated successfully to Database: {}", savedProduct.getProductId());
		return new ProductDto(savedProduct);
	...
	public void patchUpdate(String brand, String productName, long price, int productId) {
		...
		if (p == null) {
			throw new ProductNotSavedException("No Product Found By Id");
		}
		pr.partialUpdate(brand, productName, price, productId);
		LOG.info("Product partially updated to Database: {}");
	}
